...
def plot_histogram(array,bins,xlim=None):
    ...
    if xlim:
        xlow,xhigh = xlim
        plt.xlim(xlow,xhigh)
    plt.title(f'Max KLD: {np.max(array)}, Min KLD: {np.min(array)}')
    plt.xlabel('KL Divergence')
...
def get_parameters_high_kld(train_list,model,percent=5):
    ...
    klds,kld_mean = calculate_test_klds(train_list,model)
    parameters,yker_list,y_list = tm1.load_training_data(train_list)
    
    num_params = int(np.floor(len(train_list)*percent*10**-2))
